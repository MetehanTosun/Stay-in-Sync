...
public class XorOperator implements Operation {
    ...
    public void validate(LogicNode node) {
        ...
        if (inputs == null || inputs.size() < 2) {
            throw new IllegalArgumentException(
                    "XOR operation for node '" + node.getNodeName() + "' requires at least 2 inputs."
            );
        }
    }
    ...
    public Object execute(LogicNode node, Map<String, JsonNode> dataContext){
        ...
        for (InputNode inputNode : node.getInputProviders()) {
            Object value;
            try{
                value = inputNode.getValue(dataContext);
            }
            ...
            if (!(value instanceof Boolean)) {
                throw new IllegalArgumentException(
                        "XOR operation for node '" + node.getNodeName() + "' requires boolean inputs, but got a " + (value == null ? "null" : value.getClass().getSimpleName())
                );
            }
