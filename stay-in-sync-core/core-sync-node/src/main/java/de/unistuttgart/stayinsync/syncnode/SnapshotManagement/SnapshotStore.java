...
/**
 * In-memory store for {@link de.unistuttgart.stayinsync.transport.dto.Snapshot.SnapshotDTO}
 ...
 *   <li>Maintain a bounded per-transformation history (deque) for recent items.</li>
 *   <li>Evict entries beyond a time-to-live (TTL) to cap memory usage.</li>
 * </ul>
 * This store is intentionally simple and thread-safe via concurrent collections since
 * it lives in a single application node. Persistence/durability is out of scope here
