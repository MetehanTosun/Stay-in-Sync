...
public class HasAllKeysOperator implements Operation {
    ...
    public void validate(LogicNode node) {
        ...
        if (inputs == null || inputs.size() != 2) {
            throw new IllegalArgumentException(
                    "HAS_ALL_KEYS operation for node '" + node.getNodeName() + "' requires exactly 2 inputs: the JSON object and a Collection of key strings."
            );
        }
    }
     ...
     * contains all keys from the specified collection.
     *
     ...
     * matching the names in the provided key collection. Returns {@code false} otherwise.
     */
    @Override
    public Object execute(LogicNode node, Map<String, JsonNode> dataContext) {
        ...
        try {
            ...
            return false; // An input is missing, so it cannot have all keys.
        }
