...
public class DevelopmentDataSeeder {
    ...
    void seedDatabase() {
        ...
        SyncSystemEndpoint sourceEndpoint = sourceSystem.syncSystemEndpoints.stream()
                ...
                .findFirst()
                .orElseThrow(() -> {
                    // Erstelle eine aussagekräftige Fehlermeldung, die uns alle gefundenen Pfade
                    // zeigt.
                    ...
                    return new IllegalStateException(
                            "GET /products endpoint not found in source system. Endpoints found: 9. Available endpoints are: ["
                                    + availableEndpoints + "]");
                });

        CreateSourceArcDTO sourceArcDTO = createSourceArcDTO(sourceSystem, sourceEndpoint);
        SourceSystemApiRequestConfiguration sourceArc = sourceSystemApiRequestConfigurationService.create(sourceArcDTO,
                sourceEndpoint.id);
        ...
        UpdateTransformationRequestConfigurationDTO updateTargetArcsDto = new UpdateTransformationRequestConfigurationDTO(
                Set.of(targetArc.id), Collections.emptySet());
        transformationService.updateTargetArcs(transformation.id, updateTargetArcsDto);
        Log.info("-> Linked Target ARC to Transformation");

        TransformationScriptDTO scriptDTO = createFinalTransformationScriptDTO(transformation.id, sourceArc.alias,
                targetArc.id);
    ...
    private TargetSystemApiRequestConfiguration createProductUpsertTargetArc(TargetSystem targetSystem) {
        ...
        SyncSystemEndpoint updateEndpoint = targetSystem.syncSystemEndpoints.stream()
                ...
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("Endpoint PUT /products/{product_id} not found"));
    ...
    private TransformationScriptDTO createFinalTransformationScriptDTO(Long transformationId, String sourceArcAlias,
            Long targetArcId) {
        String scriptCode = """
                /**
