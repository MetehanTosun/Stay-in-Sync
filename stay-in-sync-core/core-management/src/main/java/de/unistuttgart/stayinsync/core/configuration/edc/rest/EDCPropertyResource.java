...
public class EDCPropertyResource {
    ...
    @GET @Path("{id}")
    public EDCPropertyDto get(@PathParam("id") Long id) {
...
@Path("{id}")
@Transactional
public EDCPropertyDto update(@PathParam("id") Long id, EDCPropertyDto dto) {
    dto.setId(id);
    var entity = EDCPropertyMapper.fromDto(dto);
    return service.update(id, entity)                                     // Optional<EDCProperty>
        .map(EDCPropertyMapper::toDto)                                    // Optional<EDCPropertyDto>
        .orElseThrow(() -> new NotFoundException("EDCProperty " + id + " nicht gefunden"));
}
