...
public class EDCRestClient {
    ...
    private Uni<HttpResponse<Buffer>> executeRequest(final HttpRequest<Buffer> request) {
        return request.send()
                .onItem().transform(response -> {
                    Log.debugf(response.bodyAsString());
                    return response;
                });
    }
     ...
     * @return Uni<HttpResponse < Buffer> from which the HttpResponse can be retrieved when the poll is finished.
     */
    private Uni<HttpResponse<Buffer>> executeRequest(final HttpRequest<Buffer> request, final String jsonContent) {
        return request.sendBuffer(Buffer.buffer(jsonContent))
                .onItem().transform(response -> {
                    Log.debugf(response.bodyAsString());
                    return response;
                });
