...
/**
 * Unit tests for the NotEqualsOperator.
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("Operation: NOT_EQUALS")
public class NotEqualsOperatorTest {

    private NotEqualsOperator operation;
    ...
    @BeforeEach
    void setUp() {
        operation = new NotEqualsOperator();
    }

    @Test
    @DisplayName("should return true when at least one input value is different")
    void testExecute_WhenOneInputIsDifferent_ShouldReturnTrue() {
        // ARRANGE
    ...
    @Test
    @DisplayName("should return false when all input values are equal")
    void testExecute_WhenAllInputsAreEqual_ShouldReturnFalse() {
        // ARRANGE
        when(mockLogicNode.getInputNodes()).thenReturn(List.of(mockInputNode1, mockInputNode2, mockInputNode3));
        when(mockInputNode1.getCalculatedResult()).thenReturn("test");
        when(mockInputNode2.getCalculatedResult()).thenReturn("test");
