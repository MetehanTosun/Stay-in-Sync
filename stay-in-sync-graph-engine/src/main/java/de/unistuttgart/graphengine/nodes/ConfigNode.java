 ...
 *   <li><b>First Execution Handling:</b> Returns false on first run while initializing snapshot</li>
 * </ul>
 * <p>
 * <b>Behavior on First Execution:</b><br>
 * On the very first execution (when no snapshot exists), this node returns {@code false}
 * and initializes the snapshot with current values. This prevents false-positive change
 * detection on startup and ensures that only actual data changes trigger transformations.
 * <p>
 * <b>Usage Example:</b>
 * <pre>
...
public class ConfigNode extends Node {
     ...
     *     <ul>
     *       <li>First execution: Initialize snapshot, don't detect change</li>
        ...
        if (this.getInputNodes() != null) {
            for (Node input : this.getInputNodes()) {
                if (input instanceof ProviderNode) {
                    ...
                    if (oldEntry == null) {
                        // First execution: Initialize snapshot without detecting change
                        newSnapshotData.put(key, new SnapshotEntry(liveValue, now));
                    } else {
                        // Subsequent executions: Check for changes
                        boolean isChanged = !Objects.equals(oldEntry.value(), liveValue);
                        if (isChanged) {
